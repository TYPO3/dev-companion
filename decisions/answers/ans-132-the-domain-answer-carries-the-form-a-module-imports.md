...
A caller with `core.core` in hand had to know by itself that the value goes into
`import labels from '~labels/core.core'`. The corpus states that one call away.
...
- The corpus already states the import. `javascript-labels` carries the form,
  `labels.get()`, and what an absent key throws, a call the caller has no reason
  to make with a domain in hand.
...
- A field rather than a longer sentence: `moduleImport`, beside `domain`, so a
  client reads the value rather than parses it out of prose. `D-KNW-005` is why
  an answer's data is where a value goes.
- Present where the answer handed over a domain and absent otherwise, which is
  how `domainOnNewerVersions` already works. On a version below the domains
  there is nothing to import, and the withheld answer already says what to write
  instead.
- One line in the text as well. The text is the answer a model reads first and
  the field is what a client composes with. Both come from the same value, so
...
- That a project extension's backend module imports the same way. The backend
  resolves the prefix for whatever domain a module asks of it, which is what
  `javascript-labels` states. No project module went into this read.

## Wrong if

- The import map prefix moves, or a module reaches a label another way. That
