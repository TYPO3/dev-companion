...
- It tested each of those against 354 resources, at up to three containments per
  resource. That inner loop, not the read, was the cost.
- `Labels::all()` took 0.05 s before `D-ANS-134` landed and 2.35 s after it.
...
times include the console call, which is the larger part of what remains.
...
nothing bounds. A project with a thousand XLF files pays what a stock
installation pays.

## Assumed

- Every consumer of a reference wants it for a resource it reports.
  `resourceDiagnostics()` is the only reader.

## Wrong if

- A caller needs the reference of a resource that no label of its query matched.
  Then the scan runs for a resource set the answer does not name, and what it
  saves goes with it.
- The walk and the read become the cost rather than the containments: a project
  whose sources outgrow its label files. Then what has to go is the read of
  every file, and a narrower resource set buys nothing more.
