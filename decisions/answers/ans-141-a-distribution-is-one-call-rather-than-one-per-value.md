...
**The read already grouped by three columns. So the one the caller names is a
fourth on the same query rather than a call per value.**
...
- `feedback/archive/2026-09-02-135112` established what 3,101 animal records
  ...
  grouped by one column. The counts-only mode is what made thirteen affordable
  at all.
...
- `typo3_record_lookup` takes `groupBy`: one column, counted per distinct value,
  on the same grouped read. It combines with `where`, which narrows the count.
- The column has to belong to the table the way a filter's columns do. A column
  the table does not have comes back as such rather than as an empty result.
- A value nothing carries is not in the answer. The distribution is what the
  table holds, and a caller reads a status no row carries off its absence. That
  is how that session found the two statuses no content element can select.
...
- That the grouped read costs what the ungrouped one did. It is one more column
  in the `GROUP BY` of a query that already had three. Nothing measured it on a
  table larger than the one reported.
- That the caller wants the states beside the total. Every group carries live,
  hidden and deleted because the page distribution does. A distribution that
...
- The values come back in the thousands. A column with one value per row is a
  list rather than a distribution. The answer would be a page of it with no way
  to say so.
