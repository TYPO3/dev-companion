...
**A `category` word that names no area of the tracker goes through a lookup as a
system extension key. The key is what a caller in a checkout holds.**

Half the keys reach no area by their own name, and the recovery costs the caller
a call each time.
...
- `feedback/2026-09-03-105601`. `typo3_forge_lookup(category="impexp")` answered
  ...
  answered 19 issues. The session stood in `typo3/sysext/impexp/` and had no
  other name for the subject.
...
- The composer title of the entry — the part before the dash in its
  ...
  extension's.
- The empty answer's own recovery works and the feedback asks to keep it. What
  it does not do is save the call.
...
- The title derivation is not the shape to build. One in six of the entries it
  adds resolves to the wrong area. An answer that reads a real area's issues
  under the caller's word is worse than the empty one they already recover from.
- The shape instead. The keys that reach no area are a curated map, and a
  command that reads it against the areas `Forge::categories()` fetches keeps it
  true. That is the arrangement `manuals:check` and `components:check` already
  have. A mapped area the tracker renames is then reported rather than silently
  unmatched.

## Assumed

- That the 55 areas the recorded answer lists are the areas today. The tool
  fetches the list live, and the record dates from the day of that run.

## Wrong if

- A second session reports the same round trip against an area the curated
  mapping would not have covered. Then the map is not the gap and the matcher
  is.
...
it where the caller's word matched no area. `categoriesUsed` names the area it
read, which is what the answer already promised to say.
...
`Backend User Interface` and `Code Cleanup`. A key mapped onto an area that
