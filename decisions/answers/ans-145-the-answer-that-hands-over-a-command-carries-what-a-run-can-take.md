...
**The caveat that a suite can take untracked work with it stands beside the
suite commands in `typo3_task_guide`. That is the answer a session runs from.**
...
- `feedback/2026-09-03-105814`. A core bugfix session took its commands from
  `typo3_task_guide`'s `testSuites` and ran about nineteen container suites over
  four untracked files under `Tests/`. It met the caveat only when it called
...
- The advice did not fit the checkout it exists for. It read "commit or copy out
  untracked work", and the core's own rules tell a session not to commit
  unasked. So the half that applies was the second one, and it was at the end of
  the sentence.

## Decided

- Step 2 of the ladder, delivery. The rule existed, was correct and was out of
  reach from where the task passed.
...
- It leads with the copy. The sentence names a commit as the other way out and
  as one a core session does not have.
- `beforeYouRun` is empty in a brief that matched no suite. A caveat about a
  run, in an answer that offered nothing to run, is what teaches a reader to
  skim the next one.
...
- A session reports that it lost untracked work after it read a brief with the
  sentence. Then the placement is not the gap and the wording is.
- The note grows into a second list of caveats under the suites, which would
  mean the risk one was not what the brief lacked.
