...
repository does to a render. Whoever renders requires the renderer and its theme
into a directory outside the checkout, so `build/` is gone and nothing about
them is in git.**
...
that copy into HTML. It stood here as a manifest, a lock, a gitignored vendor
directory and four steps of a command.
...
- Requiring the theme into an empty directory resolves in 3.4 seconds on a warm
  ...
  manifest and lock bought, it was not time.
...
- Resolved into this package's own `require-dev` the renderer would add 34
  packages to every `composer install`. That is 84 in the root, 56 in the
  renderer, 118 together, and no conflict. A directory outside the checkout
  costs none of that and needs no manifest here to say so.

## Decided

- `documentation:render` is gone, and `documentation:prepare` is what replaces
  it. `documentation:publish` arrived for the dark twins in the same change and
  went again when it turned out nothing referenced them.
...
- `documentation:preview` is the same sequence for one machine, and is what
  ...
  `.site/renderer` where there is none and reuses it after, because a preview
  runs again after every paragraph. Deleting `.site/` is how a preview takes the
  theme as it stands; a deployment always does.
- The requirement names the theme as `dev-main` and no lock exists, so a render
  takes it as it stands. It is this account's own repository, and a constraint
  replaces `dev-main` the day it carries a tag.
...
- A preview shows a theme nobody deploys, because `.site/renderer` is months old
  and nothing says so.
...
constraint and takes its newest release. That holds in `documentation:preview`
and in the workflow alike. Somebody would have to raise a constraint by hand
each release, and the day one came up the maintainer said to name none. What
this changes: the third **Wrong if** has its answer, because a release is a
commit somebody can go back to. `rm -rf .site/renderer` is still how a preview
