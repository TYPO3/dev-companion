...
- `typo3_record_lookup` reads the rows of a table a project-owned extension
  registers, and no recording had that answer. The recording takes its
...
- A session ran the queries by hand against the live `E-SITE` 14.3 on 2026-09-01
  and they answered. So what was absent was the environment rather than the
  code.
- Contract cases name `E-SITE` with a package of the project's own in it: "in
  the project's site package", "with the extension under `packages/`".
  `META-03`'s prompt names `packages/acme_events/Classes/` outright. The
  environment this repository makes had none, so every one of those ran against
  something the runner had to arrange.
- Written and installed against the live 14.3 on 2026-09-02: three `ddev` steps,
  seconds each. The tool then answered 125 rows on one page with the verdict
  that names the page count.

## Decided

- **The extension is `acme_events` with `tx_acme_events_event`**, which is the
  key and the table the fixture registers and the recorded call asks for. So one
  call gets its answer from both roots: for real in the environment, and as the
  boundary in the fixture, which holds no rows.
...
- **The steps run after a resume as well as after a build.** An environment from
  before this gains the extension on a second request. Each of the three says so
  and changes nothing where it is already done. That is cheaper than a state
  read off the directory, which can be wrong about a run that stopped halfway.
...
- That an installation with one package of its own is closer to what a case
  meets than one with none. `D-EVI-001` is about no scaffold of the repository a
  review then finds its defects in, and this environment is never that subject
  (`D-EVI-004`).
- That the root page the setup creates is uid 1, which is what the rows go to.
- That a table of one shape is enough. What it shows is a label, the two flags
  and a page. A table whose label column is a code, or whose rows spread over a
  tree, is a shape nothing here records.

## Wrong if

- A case run in `E-SITE` reads `acme_events` as the project's real work and
  reports about it. That would make the environment the subject rather than the
  ground.
- The three steps turn out to cost more than seconds on a resume, which is what
  keeps a second request for an environment cheap.
- A recorded answer somewhere else changes because this table is now in the
  installation. That change reads as a property of TYPO3 rather than of what
