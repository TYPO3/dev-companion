...
- What the corpus holds about impexp is three statements, and none of them says
  ...
  artifact needs, and `initial-content-import-once` is the registry row that
  stops a second import. `initial-content-references` is which relations survive
  one and what `Import::processSiteConfigurations()` rewrites.
- Everything the feedback claims about the write path holds, read in
  ...
  translation into `header.pagetree` under its own uid. A comment says the uid
  index is there so the record counts as `insidePageTree`. The node carries a
  `uid` and no `subrow`.
...
- `Import::writePages()` at line 807 takes each page's pid from
  ...
  line 1079 as each page arrives. So a page reached before its parent lands on
  the import target rather than in the tree.
- `addSingle()` unsets `pid` where the record maps to a uid that exists and sets
...
- `writePagesOrder()` at line 859 returns unless `$this->update`, and
  `$pagePid >= 0` guards its move. So the nodes with `-1` are outside the
  correction pass as well.
- The statement is unbound. The same five mechanisms stand on `.checkouts/12.4`
  at `31f881a212` (`Export.php:273`, `Import.php:727`, `781`, `793`, `1006`).
  They stand on `.checkouts/13.4` at `50cbcb5bef` (`Export.php:198`,
  `Import.php:741`, `793`, `805`) and on `.checkouts/14.3` at `fdef40e2f8`,
  whose lines are main's.
- The feedback's own line numbers are one or two off these, `1084` for the
  `importNewIdPids` assignment and `907` for the guard. It read the installation
  it worked in. The methods are what the hint can name.
- The adjacent half is a tool the session did not call rather than an answer
  that is absent. `typo3_schema_lookup` answers which field carries a
  translation parent from the installation's own TCA, and the corpus already
  carries `l18n_parent` for `tt_content` in `page-rendering.json`. A session
  that reports the tools it loaded and never called is its own subject. The
  corpus carries that shape from another checkout as
  `feedback/2026-08-24-140421`.
...
- `normal` rather than the `low` the card arrived at. A query on the import
  route reaches the hint about the export. The symptom the placement rule
  explains is records that arrive on the import target instead of in the tree.
  `initial-content-references` already serves it from the reference side.
- Not `high`. One session reported it, the task behind it was a core bug report,
  and nothing waits on the sentence.
- A hint of its own rather than more bullets on `impexp-artifact`. That hint
  answers how a session writes the artifact. This one answers why the imported
  records are where they are, which is `D-KNW-030`'s split by the question a
  caller arrives with.
...
- Neither archived nor trimmed. Nothing below `knowledge/` or `skills/` answers
  any part of the observation today, and the adjacent half takes no card here.

## Assumed

- That the symptom the session verified, page translations that arrive on the
  import target, follows from these five mechanisms. Nothing here ran an import.
  This run read the code, not a run.
- That the double use of `header.pagetree` stays. This run searched no changelog
  for a rewrite of it, and the export comment reads as the reason for that
  build.
...
- A session gets the written statement and still reads `Import.php` to find
  where its records went. The gap would have been the shape rather than the
  knowledge, and a hint would have answered a rung it does not reach.
- The statement turns out to be reachable only from a core-contribution query.
  ...
  hints, and its place is what this entry got wrong.
