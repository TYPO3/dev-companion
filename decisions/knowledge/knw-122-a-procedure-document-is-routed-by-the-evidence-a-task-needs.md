...
places that name it. A session that reviewed a PHP diff read one of those gates,
...
- The diff under review was PHP throughout — three lines in
  ...
  rendering contained. What the page sits under and what needs it are two sets,
...
- The `whenToUse` never reached this session. `ProjectDescribe::guides()` lists
  ...
  rendered none. The gate the session read is the skill's.
...
- The page's boundary does not move. It stays the proof rather than the render,
  and it names no patch. What is added is how a rendering is made to say which
  part of it changed.
...
covered version, so a probe reading it from inside one measures the moment
