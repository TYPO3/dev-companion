...
- The miss reproduces. `bin/cli hints:probe` on the feedback's own subject
  reached `javascript-unit-tests` alone. That subject is a backend Lit step that
  renders a `Task` and declares an error render branch. That hint is about how
  to cover a module rather than how to write one. No file below `knowledge/` or
  `skills/` named `@lit/task` before this change.
- The TYPO3 claim holds, read in `.checkouts/main`. All five steps of the
  localization wizard declare an `error:` branch that calls
  `this.context.wizard.renderError()` with a label of its own. All five task
  functions close on `catch (error) { console.warn(...); return []; }`. The
  branch cannot fire, and the caller sees the empty-result markup of the
  `complete` branch instead.
- The shape that works stands in the same tree.
  ...
  icon out of `error:`. So the rule comes off the core rather than out of an
  argument from the library.
...
- A measure settled the queries in reach rather than an assumption.
...
- That the caller arrives while writing or debugging the error path, which is
  ...
  that arrives from the rendered symptom, an empty list where a request failed,
...
- That the core keeps rendering asynchronous backend data through `@lit/task`.
  ...
  is the trend the hint stands against.

## Wrong if

- A core `Task` returns an empty value on failure on purpose and renders an
  empty state for it. Then the catch is a choice rather than a defect, and the
  hint's second statement claims more than the tree supports.
- A session reports the same cost with the hint in place, after a search by the
  wizard rather than by the component. Then `appliesTo` has the wrong vocabulary
  and the assumption above is what failed.
- The backend adopts another shape for asynchronous rendering. Then the hint
  describes an API the tree no longer writes. What replaces it is a new
