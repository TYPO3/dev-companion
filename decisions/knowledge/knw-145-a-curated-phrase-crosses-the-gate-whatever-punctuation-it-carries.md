...
- `fluid-object-access` carries `is of type "boolean"` and
  ...
  read them as no phrase at all and the hint stayed behind the gate for a
  caller standing in the class that raised the message — which is what
  `D-KNW-075` measured from the other end on 2026-09-02 and answered for that
  one message by putting a keyword in the Fluid domain.
...
- The sweep the todo asked for, over 206 queries — every forward and contract
  ...
  for entry.
- What changes is the case it was written for. With `Classes/Domain/Model/`
  in the paths and `is of type "boolean"` in the task, the answer went from
  `extbase-domain-mapping` alone to that hint and `fluid-object-access` beside
  it. Two other symptom queries measured the same way are unchanged, because
  the hints that answer them are in the domain the paths already selected.
...
- The shape test is two rules rather than a regex: a pattern carrying a path
  ...
  hint is filed under. Both were already the rules `D-ANS-084` decided; only
  the second was expressible in the pattern it used.
...
- The path half is stated in the code and held by no test. Every query that
  ...
  path pattern without the gate having already selected that hint's domain —
  and a test asserting it would pass with the rule deleted.
...
- That 33 more phrases behave like the 809. The sweep says they change nothing
  on the queries this repository has; a corpus that grows its message
  vocabulary is the case nobody has measured.
...
- The crossing becomes ordinary rather than an exception as messages are
  curated onto more hints, which `bin/cli hints:coverage` and the sweep are
  what would show.
