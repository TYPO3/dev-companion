...
**A session reads a statement about Fluid in `.checkouts/fluid/<line>`, at the
line the core's own `require` pins.**
...
- `BooleanNode` is in `typo3fluid/fluid`, which no core checkout contains. Each
  ...
  `main` `^5.3.2`. Every `composer.lock` beside those manifests names
