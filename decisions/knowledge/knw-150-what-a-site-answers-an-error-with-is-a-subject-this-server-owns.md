...
- `feedback/2026-09-03-235538`. A project audit found `errorHandling` empty and
  repaired it from memory, with nothing here to confirm it at v14. That is
  `errorCode`, `errorHandler: Page`, `errorContentSource` as a `t3://page?uid=`
  reference. It called `page-not-found-within-a-site`, which is diagnostic and
  names the configuration as the discriminator and does not say what it holds.
- Read in `.checkouts/12.4`, `13.4`, `14.3` and `main`. `Site::__construct()`
  ...
  falls back to the entry with code `0` before it throws
...
- Its three failure texts are in that class. The DoS guard where the error page
  is the address in the request, and the `RuntimeException` where the subrequest
...
- Step 1a. `site-error-handling` is a hint of its own rather than statements on
  the diagnostic one. To read what a not-found was and to configure what answers
  it are two questions. The first now closes with the name of the second.
- Two statements carry a bound: `LoginRedirect` and the Extbase settings, both
  `since: 13`.
- The status codes stand as `HTTP 404` and `HTTP 403` rather than bare.
...
- The hint states `nav_hide` against `hidden`. The practice the feedback
  reports, an error page kept out of the menu, is one word away from the setting
  that breaks the handler.
...
- That `errorHandler` has no fifth value on a branch this reads. The four are
  ...
  would arrive as a failed lookup rather than as a wrong answer.

## Wrong if

- A session reports that the internal subrequest reached a page an anonymous
  visitor cannot see, which would make the deliverability statement wrong.
- The core no longer reads the fallback on `errorCode` 0 before the exception,
  which is the one behaviour here that no message names.
