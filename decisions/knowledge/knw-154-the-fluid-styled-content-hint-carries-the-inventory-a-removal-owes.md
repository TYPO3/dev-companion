...
**The hint that already names the object a site takes the dependency for states
what the site owes when it drops `fluid_styled_content`.**
...
- Routing and delivery are not where it failed.
  ...
  ranks the hint first. `typo3_task_guide` returned it unasked. That is ladder
  step 1a: the answer is not there.
...
- `fluid_styled_content` declares `styles.templates.*` and `styles.content.*`
  and no other system extension does. `ConstantAwareTokenStream::__toString()`
  falls back to the token where nothing declares one. So a reference that
  survives becomes literal text rather than an empty value.
...
- The statements go into `sitepackage-fluid-styled-content` rather than a
  ...
  what its removal costs arrive at the same object. A split would give the
...
- The `list` subtype statement carries a bound rather than stands as the rule,
  because `ContentElement/List.typoscript` is gone on the newer branches.
- The feedback's checklist frame stays out. A hint states what holds and the
  order a session notices a failure in is the guide's. So the plugin statement
  leads and the CSS one keeps the place it already had.

## Assumed

- That a site that drops the dependency defines `lib.contentElement` itself,
  which is what the hint's closing statement already recommends. A site that
  removes the object outright meets the silent case instead, and the hint states
  both.
...
- A session reports that it rebuilt `lib.contentElement` and still missed one of
  the four. That would mean the inventory is a document rather than a run of
  statements.
