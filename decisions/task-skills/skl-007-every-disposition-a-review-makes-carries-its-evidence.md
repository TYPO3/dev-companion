...
- The second recorded `REVIEW-03` run carries all three dispositions and asked
  for none of them. It discounted two `typo3_commit_message_guide` warnings and
  named why they do not hold. It attributed twice, established that the parser
  the patch adds already exists on `origin/14.3` and marked the empty-tab
  finding new. It reported the security surface as assessed while it never made
  the finding of the run before it, four fields written past the `$fieldList`
  guard. Two behaviours that nothing required, and one failure that nothing
  caught, in one run judged as one that meets every criterion.
- That run's own judgement names the third as a shape rather than an accident.
  ...
  still needs looking at". That is the shape `REVIEW-02` was partial for and
...
- A published multi-pass review pipeline for a large C project, read on
  ...
  candidate.
- The same pipeline marks every finding as introduced or pre-existing and
  requires the reported wording to state it. It keeps pre-existing findings out
  of the report below its top two severities. It runs a benchmark of its own for
  that one property.
...
- A surface reported as assessed names what the review read to assess it. Where
  the read did not happen the word is *unassessed*, which costs the same line
...
- The reviewer makes the author's case against a finding before the report, and
  reports what survives together with what it survived.
...
- That a core patch set is small enough that a read of the later patches to
  settle a finding is affordable. The pipeline this came from reviews series of
  a size that made automation worth it.
...
The section covers the core patch review alone, on the read that an audit would
raise more candidates than a reader sits through. The two recorded conformance
runs say otherwise: each carries four dismissals in prose, none of them asked
for. So the requirement holds over both checklists and the conformance one
gained the same section, with one boundary the patch review does not need. The
coverage list answers a subsystem the package does not ship, rather than a
dropped candidate. The volume argument stays worth a watch in the direction it
came up for.
