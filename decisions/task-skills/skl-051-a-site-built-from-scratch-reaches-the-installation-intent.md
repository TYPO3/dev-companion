...
**The words a brief names the installation with reach `installation-setup`, and
not only the verb that sets one up.** A site asked for from scratch matches it,
and the installation named as a unit of the work matches it weakly. Nothing in
`src/` changed: a brief carries as many intents as it recognizes and always did.
...
- **One intent was never all the answer can carry.** `TaskGuide::answer()`
  ...
  of each confirmed one. Measured on the same day, the guide recognized the
  brief for `labels` as `labels` and `backend-ui` together, both strong.
- **The scope read is not what withheld the route.** `installation-setup`
...
- **The matcher was not in the way either.** `Text::containsWord()` matches a
  needle's inflections and reads a compound with a space or a hyphen in it. So
  `development installation` in the brief would have matched a needle of that
  name. There was none.
- **The wider net moved one line in 61.** This entry matched one brief per
  intent, nineteen, and every prompt in `scenarios/`, forty-two of them, before
  and after. The only difference is the compound brief, which now answers
...
- **The same miss is in a contract case.** `SITE-01`'s prompt opens "Fresh TYPO3
  here. Set up the site" and matches no installation intent either. It stays as
  it is. That case states the installation steps as not covered, and it predates
  the publication of `typo3-development-installation`.
...
- **Two needles rather than a general one.** `from scratch` on its own reaches
  ...
  swallowed neighbour the measurement above is for.
- **The third unit of the brief stays unrouted.** The production of a
  distribution's content has no intent, because it has no skill yet. That is
  `D-SKL-050`'s card, and this entry does not anticipate its words.

## Assumed

- **That a site asked for from scratch always begins with an installation to
  make.** Nothing measured says how often a caller writes the phrase about a
  site that is already up.
- **That the nineteen briefs stand in for what a caller writes.** They are this
  repository's own sentences, one per intent. The forty-two that somebody wrote
  to describe a task rather than to guard one are the scenario prompts.

## Wrong if

- A brief about work inside a site that already exists says "site from scratch".
  It lands in the installation workflow, with its checklist stated as fact.
- A compound brief matches only the weak `development installation`, and the
  ...
  the route is what the feedback said was the gap.
