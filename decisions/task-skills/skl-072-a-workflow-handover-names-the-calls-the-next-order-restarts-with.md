...
- The clause lands and a session skips `typo3_documentation_lookup` again on a
  ...
  body at its point of use, which a crossing clause does not reach, and it would
  say the pattern is wider than the handover.
...
is that nothing can reach the **Wrong if** until the trigger holds, and the
