...
- **The session that saw its own routing.**
  ...
  heredocs that did exact string replacement in PHP files. Each reimplemented
  the match-uniqueness check the client's own `Edit` performs, and one of them
  wrote a `use` for a class that does not exist.
- **The cause is a harness instruction and not a judgement about any tool.** The
  ...
  job, edits included. `Edit` stood loaded, first-party and free to call, and
  lost anyway.
- **The order of the two costs.** A tool of this server's arrives in that client
  as a deferred name. So a call to one costs a `ToolSearch` round trip before
  the call. The report states the consequence rather than infers it: the
  cheapest server call cost strictly more than the grep it would have replaced.
- **What the same burst shows on the knowledge side.** Three of its reports name
  a fact as absent that this server already carries and already delivers.
  `typo3_test_run_guide` emits the note that `runTests.sh` stops to read its own
...
- **A session weighs a lever on the publication side against this bound before
  it builds one.** Rewriting a description, adding a trigger word, moving a
  sentence: all of them address which tool a session picks, and this session
  picked none.
...
- **A session that made no call is not evidence that the answers are poor.** It
  ...
  tools a session reached, never which were worth the reach.
...
- A session reports that it reached a tool of this server's from inside a
  Bash-first routing. Then the bound is softer than this entry states and the
  wording levers are worth their cost again.
- A report names a client that loads these schemas up front, and a session there
  still routes past them. Then the round trip is not what decides it and this
  entry names the wrong cause.
- The instruction turns out to cover reads and searches alone in the clients
