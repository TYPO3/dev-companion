...
.. code-block:: yaml
    ...
    # One of: system, project, third-party, fixture. system: TYPO3's own. project:
    # inside the repository. third-party: installed as a dependency. fixture: below
    # a Tests/ directory, so it belongs to the test setup.
...
- ``key``, ``path``, ``origin``, ``tcaTables``, ``tcaOverrides``,
  ``contentElements``, ``renderedContentTypes``, ``pluginFrame``,
  ``unlistedFlexForms``, ``backendModules``, ``icons``, ``siteSets``,
  ``formConfigurations``, ``serviceTags``, ``files``, ``deprecatedFiles``,
  ``notReadStatically``, ``artifacts``, ``answeredBy``
- ``key``, ``unsupported``
