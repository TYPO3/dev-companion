...
Trimmed on 2026-09-02: the cheap half this names is done. typo3_backend_module_lookup takes a `file` and says which parent and which iconIdentifier it names that this installation does not have, without a cache flush and without the file being installed. The labels domain is read and not resolved, and the resolution half — route paths, whether an entry gets routes, sibling placement — is what is left. `D-FBK-055` says why it waits.
