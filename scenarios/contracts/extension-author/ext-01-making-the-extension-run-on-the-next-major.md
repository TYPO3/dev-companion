...
- How to find out what changed: the changelog directory, the extension scanner,
  the deprecation annotations. That is a procedure that works on any branch, not
  a list from one.
- Which conventions the new code has to follow where the session rewrites it.
  DI, event listeners instead of hooks, the TCA and schema shape, the Fluid
  changes.
...
- The conventions that transfer come over with a mark as conventions rather than
  as core rules. Nothing that only exists in the core repository goes to a
