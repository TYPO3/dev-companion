...
A session this server did not design decides what it is worth. Someone asks an
agent to review their repository, the agent decides what matters and reaches for
whatever tools it has. The answer is either useful for that person or it is not.

Two different tests once shared the name scenario here, and the name hid the
difference:
...
- [Targeted contract cases](contracts/readme.md) name one task shape or failure
  mode so a test can hold its routing and workflow still. They are intentionally
...
is the unit a session reads, runs, and edits both kinds in. A file that holds
several prompts is a file where nobody can tell which criteria the judgment
covered.
...
The same person is routinely two of them on the same day. That is why the
...
A case that needs a console command names it. `E-SITE` is the environment a
session asks `language:domain:search` of, which is a version question rather
than a property of the directory.
...
[todo/reference/](../todo/reference/), where it can go stale without a case in
tow. A prompt that names somebody's project is a prompt only that person can
run.
...
database, a docker daemon a CI job may not have. This repository makes it
because everything a case needs from it is a property this repository can state.
One installation runs one version, so there is one per covered version. Named
none, the covered stable one, which is what a case that says nothing about a
version runs on. `SITE-02` is the case that says otherwise, and
...
that belongs to one of them could go nowhere
...
find in it is what this repository put there. That is the thing
...
- A contract case: [contracts/readme.md](contracts/readme.md).
  `bin/cli scenarios contract <id>` prints it for a look. Nobody can record it
  as a forward run.
