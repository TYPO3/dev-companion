...
- `installation-boot` — a base whose host is not the host the site is served
...
- `autogenerated-site-configuration` — a site nobody wrote. The core writes one
  when a page is created at the root, on a sub-path of the URL the request that
  created it arrived on. Which of the last two left a site is read off its
  identifier rather than off the symptom, because the two produce the same
