...
final class CommitMessageGuide extends ReadOnlyTool
{
    ...
    {
        return 'Draft and check a TYPO3 commit message. A person who wants to know what the commit did reads the message. So write it in plain English and only as long as that answer needs; the diff carries the detail. Either assemble one from parts (keyword plus summary) or pass an existing message to check and correct it. The returned draft is ready to commit. Its body wraps at 72 characters. The checks name every run of lines the wrap joined and every line it could not bring under the width. Defaults to a repository of your own, where the subject and body conventions apply and no Forge issue, Releases: trailer or changelog is demanded. The draft still writes the issues you pass as Resolves: and Related: trailers there. That is the form a TYPO3 repository on GitHub links a commit to what it closes by. Pass workflow="core" for a patch against the TYPO3 core, where the Forge issue and the Releases: trailer are required. Where the call carries none, the answer names the branches for that trailer. That is the lines that take a patch today, and the ones a change of this shape goes to. A change still in work says so with workInProgress or with [WIP] in its own subject. The Forge issue is no error there, since the merge is what requires the trailer. A core body that counts what the change touched, so many files or so many spellings, gets a check that says so. The core\'s own bodies do not count.';
    }
    ...
    public static function inputSchema(): array
    {
        return [
            ...
            'properties' => [
                ...
                'workflow' => ['type' => 'string', 'enum' => ['core', 'project'], 'default' => 'project', 'description' => 'Which rules to apply. "project", the default, is any repository of your own. The checks read the keyword, the 52/72 character limits and the wrap. They demand and invent no trailer, the draft writes the issues you pass out all the same, and [SECURITY] is allowed. "core": a patch against the TYPO3 core, with the Forge issue and the Releases: trailer required.'],
                ...
                'body' => ['type' => 'string', 'description' => 'Optional commit body, for what the diff does not say: why the change was made, what it rests on. The draft wraps it at 72 characters. Indent a block to keep the line breaks you wrote, and keep those lines under the width yourself.'],
