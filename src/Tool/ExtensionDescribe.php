...
final class ExtensionDescribe extends ReadOnlyTool
{
    ...
    {
        return 'Describe what one installed extension registers. That is the tables its TCA defines and extends. The content elements it adds to tt_content, with the template each renders through and the FlexForm each binds. Its backend modules and routes, and its icons. Its site sets and the files each carries. Its form configurations and the form definitions they store. Its service tags, middlewares, Fluid roots and global Fluid namespaces. The shape of its Classes/ directory. The files it ships that the core no longer reads, with what each costs; four predicates, so not an upgrade check. And beside all that its manual, README, test layers and XLF files. Tables, content elements and icons come from the booted installation where there is one. Everything else comes from its own files and never executes, so it answers on a fresh clone and for a third-party extension too. Call it with a key typo3_project_describe lists. A key the installation does not have gets the keys it does have as its answer, and no installation at all gets unsupported. It answers what the extension registers and never what editors store through it. How many rows one of its tables holds, on which page and what they are is typo3_record_lookup.';
    }
    ...
    public static function outputSchema(): array
    {
        return Schema::installationAnswer([
            ...
            'tcaTables' => Schema::listOf(Schema::string(), 'Tables its Configuration/TCA/ defines. Where the booted installation answered, the EXT: reference each entry carries attributes it to this extension. So a table added by a PHP call is among them. Otherwise by file name. typo3_schema_lookup takes one of these names and answers what columns the core derives for it.'),
            ...
            'contentElements' => Schema::listOf(Schema::object([
                ...
                'flexForm' => Schema::nullableString('The FlexForm data structure it binds, as the call declares it. That is a FILE:EXT: reference, or "inline" where the XML stands in the override file itself. Null where it binds none, which is a different element to review than one that does.'),
            ], ['identifier', 'kind', 'templateName', 'source', 'pluginSettings', 'flexForm']), 'The content elements it adds to tt_content, where each renders, and what it configures through. Where the booted installation answered, the identifiers are what it registered, attributed to this extension by the EXT: reference each carries. So an element whose identifier came out of a variable is among them. Otherwise they come from the override files, as identifier says.'),
            'renderedContentTypes' => Schema::listOf(Schema::object([
                ...
                'registeredBy' => Schema::nullableString('The extension whose TCA registers that CType, read off the EXT: reference in its label. Null where no installation answered. On a booted one, null is a definition for an element nothing registers: an editor cannot select it and the file is dead.'),
            ], ['identifier', 'templateName', 'source', 'registeredBy']), 'Content types this extension renders and does not register, which is what a package that takes the render frame over from fluid_styled_content ends up with. The deletion of one of these definitions leaves an element editors can still select with nothing to render it. No other field of this answer names it. Empty where the extension renders only what it registers.'),
            'pluginFrame' => Schema::nullableString('The Generic template this extension ships, relative to it, or null where it ships none. ExtensionUtility::configurePlugin() renders every Extbase plugin registered as a content type through templateName = Generic, so a package that defines lib.contentElement itself owes this file. Nothing in the package points at it, which makes it the one a cleanup deletes as unused first. Its deletion empties every plugin on the site.'),
            'unlistedFlexForms' => Schema::listOf(Schema::object([
                'identifier' => Schema::string('The content type the bound structure names.'),
                'flexForm' => Schema::string('The data structure, as above.'),
            ], ['identifier', 'flexForm']), 'FlexForm bindings read from the override files whose content type none of the contentElements entries above carries. typo3_flexform_lookup resolves one to its fields. Usually empty. An entry here is a registration this answer read and could not attribute. The identifier is real and the bound structure is real, and the tool did not establish whatever else registers that element.'),
            ...
            'icons' => Schema::listOf(Schema::string(), 'Identifiers from Configuration/Icons.php. Where the booted installation answered, every icon it registered below this extension\'s EXT: reference, a list built in a loop included. typo3_icon_lookup searches every package at once.'),
            'siteSets' => Schema::listOf(Schema::object([
                ...
                'files' => Schema::listOf(Schema::string(), 'Which of the files core reads a set directory for are in it: settings.definitions.yaml, settings.yaml, route-enhancers.yaml, labels.xlf, page.tsconfig, constants.typoscript, setup.typoscript and include_static_file.txt. config.yaml is not among them, because it is what makes the directory a set. The core reads route-enhancers.yaml from v14.1; on v13 it loads a set that carries one and ignores that file. The last four are the defaults a set gets where its config.yaml declares no typoscript, pagets or labels path of its own. One that declares them reads from there instead, and this list does not say so.'),
            ], ['name', 'path', 'files'])),
            ...
            ], ['path', 'name', 'registeredBy', 'storagePaths', 'formDefinitions']), 'The form configurations it registers, both ways in. Empty where it registers none. An extension that ships a .form.yaml and registers no storage for it has a form nothing loads, which is what you read this list for.'),
            ...
            'classes' => Schema::object([
                ...
                ], ['name', 'files']), 'Every directory directly below Classes/, whatever its name. Nothing filters them, so a directory here is not a registration of any kind; it is what the extension calls it.'),
                'looseFiles' => Schema::integer('PHP files directly in Classes/, under no directory of their own.'),
            ...
            'deprecatedFiles' => Schema::listOf(Schema::deprecatedFile(), 'The files this extension ships that the core no longer reads, or stops to read, each with what the shipped file costs. The check reads four predicates, each a file and what stands beside it. ext_tables.php. ext_emconf.php beside a composer.json that declares neither providesPackages nor a version. ext_icon.svg/.png/.gif where no Resources/Public/Icons/Extension.* stands for the core to read first. An ext_typoscript_*.txt with no .typoscript file of the same name beside it. The check reads each from the extension\'s own tree, which is where every one of these predicates lives. So no changelog sweep over what its code calls reaches any of them. An empty list says none of the four holds here, not that the extension is ready for the next major. The check reads nothing else here for a deprecation, and typo3_changelog_lookup answers that question.'),
            'notReadStatically' => Schema::listOf(Schema::string(), 'Declaration files that are there but whose entries do not stand in their own text. Each assembles its list while it runs, so the lists above lack what it registers rather than say it is absent. The booted installation is what answers for them. An empty list says each declaration file that exists stood in its own text, not that the tool read everything the extension ships. ext_localconf.php and ext_tables.php register at run time, and nothing here reads them.'),
            ...
            ], ['manual', 'readme', 'tests', 'languageFiles'], 'What it ships beside its registrations. Every key is present even when the artifact is not. The absence of a manual, a test or a translation is the answer a file listing cannot give.'),
