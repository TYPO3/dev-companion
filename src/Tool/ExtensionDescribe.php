...
final class ExtensionDescribe extends ReadOnlyTool
{
    ...
    {
        return 'Describe what one installed extension registers: the tables its TCA defines and extends; the content elements it adds to tt_content, with the template each renders through and the FlexForm each binds; its backend modules and routes; its icons; its site sets and the files each carries; its form configurations and the form definitions they store; its service tags, middlewares, Fluid roots and global Fluid namespaces; the shape of its Classes/ directory; the files it ships that core has stopped reading, with what each costs — four predicates, so not an upgrade check; and beside all that its manual, README, test layers and XLF files. Tables, content elements and icons come from the booted installation where there is one. Everything else is read from its own files and never executed, so it answers on a fresh clone and for a third-party extension too. Call it with a key typo3_project_describe lists. A key the installation does not have is answered with the keys it does have, and no installation at all with unsupported. It answers what the extension registers and never what is stored through it: how many rows one of its tables holds, on which page and what they are is typo3_record_lookup.';
    }
