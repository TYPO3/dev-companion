...
final class FeedbackRecord implements Tool
{
    ...
    public static function inputSchema(): array
    {
        ...
        return [
            'type' => 'object',
            'properties' => [
                'subject' => ['type' => 'string', 'description' => 'One short line saying what only this feedback reports, in English — "a release branch\'s log answers about the history from before it was cut". It becomes the title and the file name, which is all a maintainer reads when deciding what to open. Left out, both come from the opening of the observation. A session that files several at once then gets several that begin on the same words. The observation opens with the task, so every feedback from one session opens alike. Say what this one says and nothing the others do.' . sprintf(' At most %d characters, and a longer line is cut to fit rather than refused.', Channel::MAX_SUBJECT_LENGTH)],
                'observation' => ['type' => 'string', 'minLength' => 1, 'description' => 'What was missing, wrong, or unhelpful, specific enough to act on later, in English. Open with one line naming the task you were given, so the feedback can be traced back to what exposed it. A finding is the path of a value, the shape of what came back, and where it came from. It is never the value itself where the installation keeps it secret: an encryption key, a password, a token, the credentials in a connection string. A commit pushes this into a checkout that installation\'s owner does not watch, so a secret pasted here as proof has left it for good. The finding is "the key at SYS/encryptionKey is the active one, hardcoded in config/system/settings.php"; the 96 characters after it establish nothing further.' . $storedCap],
