...
final class HintLookup extends ReadOnlyTool
{
    ...
    {
        return 'Return hints for TYPO3 core paths or task topics, grouped by section. A hint is a convention at the code itself: what to write at a path, which idiom a subsystem uses, what a finding costs. A procedure in steps is typo3_rule_lookup instead. That is the commit message conventions, the changelog entry a change owes, the Gerrit push and amend workflow, the setup of an extension manual. The checklist a change owes, with the hints its paths match beside it, is typo3_task_guide. Where the paths read as a project or third-party extension the hints still come back, because the conventions transfer. The "Backend CSS" and "Backend TypeScript and JavaScript" sections describe the TYPO3 backend interface. The answer withholds them, with the reason, where the task names the frontend. The subjects are ' . implode(', ', Hints::subjects()) . '.';
    }
    ...
    public static function inputSchema(): array
    {
        return [
            'type' => 'object',
            'properties' => [
                'paths' => ['type' => 'array', 'items' => ['type' => 'string'], 'default' => [], 'description' => 'File paths related to the task, as they are in the repository they belong to. The answer places each on its own, so a core path and an extension path in one call match separately. It labels a statement where it obliges the other one.'],
                'task' => ['type' => 'string', 'description' => 'Short task description or topic, in English. A symptom is a query this takes as readily as a subject. The search reads a hint by its own statements. A phrase in its index reaches it from the layer that explains the failure rather than the one the failure showed in. Matching is lexical against English text, so another language reaches only the loanwords.'],
                'id' => ['type' => 'string', 'description' => 'Ask for one hint by its id, for example language-files, instead of a match. Every answer lists the ids it did not return. So you request a subject a query missed by name rather than guess at it in other words.'],
                'targetVersion' => ['type' => 'string', 'description' => 'The TYPO3 version the answer has to hold for, for example "13.4" or "14". The answer leaves out statements that do not hold there, those the repository needs for another major it declares included. Defaults to every major this repository declares typo3/cms-core for, or to the installation this server started in where there is no declaration. Where there is neither, nothing filters and every statement carries the versions it holds for.'],
                'limit' => ['type' => 'integer', 'minimum' => 1, 'maximum' => self::MAX_HINTS, 'default' => 6, 'description' => 'Maximum number of hints.'],
                'availableHints' => ['type' => 'boolean', 'default' => false, 'description' => 'Ask for the index of the ids beside it on a call that names an id. The answer withholds it there by default. A caller who names an id has already chosen, and the list was two thirds of what such an answer carried. A call that matches by paths or task carries it either way, and so does an id that matched nothing.'],
            ],
        ];
    }
    ...
    public static function outputSchema(): array
    {
        return Schema::object([
            ...
            'scopes' => Schema::scopes('Which kind of work each path is. Paths of different scope match separately, so a hint that came back for one of them is about that path.'),
            'targetVersion' => Schema::nullable(['type' => 'integer', 'description' => 'The TYPO3 major this repository runs — stated by the caller, or read from the installation. Null means no version filtered anything and every statement carries its own range. Where the repository serves several majors, targetVersions is what the answer holds for.']),
            ...
            'domains' => Schema::listOf(Schema::string(), 'A hint outside these domains comes back only where the task spells out a phrase in its index and no hint inside them claims it. That is how a symptom reaches the layer that explains it.'),
            'withheldCategories' => Schema::listOf(Schema::string(), 'Categories that matched the domains and stay out because the task names the frontend. "Backend CSS" and "Backend TypeScript and JavaScript" describe the TYPO3 backend interface and are wrong advice for what a website renders. See docs.typo3.org for a frontend theme.'),
            'hints' => Schema::listOf(Schema::hintRecord()),
            'bestCoverage' => Schema::nullable(['type' => 'number', 'description' => 'How much of the task the closest hint above carries, between 0 and 1, where 1 is every word of it. A hint answers on its own from 0.5. Below that it is here because it claims one of the paths you named or a phrase somebody anticipated. That is not the same as an answer to the question. Six well-formed hints that all got in that way read exactly like six that answer you, and this is what tells them apart. Null on a call that named an id, which is not a guess at anybody\'s words.']),
            'availableHints' => Schema::listOf(Schema::hintReference(), 'The hints that exist in the searched domains, minus the ones above, closest first. What the limit cut stands before what matched too little to return. That order is the matcher\'s, so it holds where a query matched, which is every call except one that names an id. An id that matched nothing lists every id there is, in corpus order. An id that matched carries this empty unless the call asked for it, and availableHintsWithheld says how many stay out.'),
            'availableHintsWithheld' => ['type' => 'integer', 'description' => 'How many ids beside it stay out of availableHints. Non-zero only on a call that named an id and matched one without a request for the index; pass availableHints true to receive them.'],
            'documents' => Schema::listOf(Schema::object([
                'uri' => Schema::string(),
                'hint' => Schema::string('The returned hint this document is the long form of.'),
            ], ['uri', 'hint']), 'Knowledge documents that declare themselves the long form of a hint above. A hint is the convention in short and the document is the same subject at length. Where it hands over a file it is the file itself.'),
