...
final class HintLookup extends ReadOnlyTool
{
    ...
    public static function inputSchema(): array
    {
        return [
            ...
            'properties' => [
                ...
                'id' => ['type' => 'string', 'description' => 'Ask for one hint by its id, for example language-files, instead of matching. Every answer lists the ids it did not return. So you request a subject a query missed by name rather than guess at it in other words.'],
                'targetVersion' => ['type' => 'string', 'description' => 'The TYPO3 version the answer has to hold for, for example "13.4" or "14". Statements that do not hold there are left out, including those the repository needs for another major it declares. Defaults to every major this repository declares typo3/cms-core for, or to the installation this server started in where there is no declaration. Where there is neither, nothing filters and every statement carries the versions it holds for.'],
                'limit' => ['type' => 'integer', 'minimum' => 1, 'maximum' => self::MAX_HINTS, 'default' => 6, 'description' => 'Maximum number of hints.'],
                'availableHints' => ['type' => 'boolean', 'default' => false, 'description' => 'Ask for the index of neighbouring ids on a call that names an id. The answer withholds it there by default. A caller who names an id has already chosen, and the list was two thirds of what such an answer carried. A call that matches by paths or task carries it either way, and so does an id that matched nothing.'],
            ],
        ];
    }
    ...
    public static function outputSchema(): array
    {
        return Schema::object([
            ...
            ], ['uri', 'hint']), 'Knowledge documents declaring themselves the long form of a hint above. A hint is the convention in short and the document is the same subject at length. Where it hands over a file it is the file itself.'),
