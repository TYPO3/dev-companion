...
final class ProjectDescribe extends ReadOnlyTool
{
    ...
    {
        return 'Describe the repository this server started in and the TYPO3 installation it has made. It answers the TYPO3 and PHP constraints, with the floor the installed core requires and how the PHP numbers stand to each other. It says whether the install is what composer.lock names. It lists the extensions that are its own rather than TYPO3\'s, and the sites it configures with the sets each depends on. It lists the commands composer.json and every package.json declare, each marked check, change or unknown. It says whether their interpreter clears the PHP bound the install wrote. It describes the environment it runs in, with its PHP, hooks and pull recipes, and which Node its npm commands run on. It lists its patched dependencies and the guides this server carries. Read from files alone — no console, no database — so it answers on a fresh clone as well. Before composer install has run, installed says so and four fields wait for it: typo3Version, corePhpConstraint, installedPhpBound and extensions. Call it first, before you boot the project and before you recommend or run a check. A check this repository does not declare does not exist here. The ones marked check are what a task told not to change files may run. What one of the extensions it lists registers — its tables, content elements, backend modules and icons — is typo3_extension_describe.';
    }
    ...
    public static function outputSchema(): array
    {
        return Schema::installationAnswer([
            'root' => Schema::nullableString('Absolute path of the repository this describes. Null when the discovery found no project root to describe.'),
            'kind' => Schema::string('core-checkout or composer-project. What the root declares itself to be, not whether anything sits installed in it; installed says that.'),
            'installed' => ['type' => 'boolean', 'description' => 'Whether the packages this repository declares sit installed below it. False is a clone nobody has run composer install in, which is the state a boot or an installation task starts in. Everything else here comes from the repository\'s own files and answers either way. What false costs is the four fields that come out of the installed tree: typo3Version, corePhpConstraint and installedPhpBound are null and extensions is empty. None of the four tells you that on its own.'],
            'installedAgainstLock' => Schema::object([
                'state' => [
                    ...
                    'description' => 'matches: every package composer.lock names sits installed at that version, so a failure here is not a stale install. differs: packages says which of them are not, and the install is behind or ahead of the lock. not-installed: there is a lock and no Composer metadata below the vendor directory to hold it against. So the packages it names are not on disk. no-lock: this root has no composer.lock, so nothing states which versions it fixed.',
                ],
                'packages' => Schema::listOf(Schema::object([
                    'package' => Schema::string('The Composer package name, as both files spell it.'),
                    'locked' => Schema::nullableString('The version composer.lock names. Null where the lock names this package nowhere and it sits installed anyway.'),
                    'installed' => Schema::nullableString('The version installed below the vendor directory. Null where the lock names it and nothing sits installed under that name.'),
                ], ['package', 'locked', 'installed'])),
            ], ['state', 'packages'], 'Whether what sits installed below the vendor directory is what composer.lock names, package by package and version by version. The one thing installed cannot say. A vendor directory a month older than the lock satisfies that boolean, and the suite run after it fails in classes your change never touched. The modification times are not read: a lock a rebase rewrote is newer than the install it describes, and nothing there is stale. The comparison takes locked dev packages only where the metadata says the install took them, so a --no-dev install is no drift. Where the two differ, the text names the command that reinstalls them. Empty packages where state is anything but differs.'),
            'typo3Version' => Schema::nullableString('The TYPO3 version installed here, read from the core package. Null where nothing sits installed yet, which installed says.'),
            ...
            'node' => Schema::nullable([
                ...
                'properties' => [
                    ...
                    'nvmrc' => Schema::nullableString('What the .nvmrc beside it says, as spelled. The closest thing here to what a developer actually runs, because a version manager reads that file and selects it. An alias like lts/iron stays as it is, unresolved. What it names is a list nvm downloads, not anything in this repository. Null where there is no such file.'),
                    ...
                    'environment' => Schema::nullableString('The Node the environment states, which for DDEV is nodejs_version. Null is not "none". A project that states none gets the default of the installed DDEV. That default is not in these files and changes from one release to the next. Also null where the environment is not DDEV, or where there is no environment at all.'),
            ...
            'environment' => Schema::nullable([
                ...
                'properties' => [
                    ...
                    'node' => Schema::nullableString('The Node that environment runs, where its files state one — nodejs_version in the .ddev configuration. Null where they state none, and then the installed DDEV\'s own default applies. The node object above holds it against what this repository declares.'),
                    'source' => Schema::string('Where this comes from: the .ddev config file that states the version last, or TYPO3_DEV_COMPANION_CONSOLE.'),
            ...
            'extensions' => Schema::listOf(Schema::object([
                ...
                'origin' => ['type' => 'string', 'enum' => ['project', 'third-party', 'fixture'], 'description' => 'project: inside the repository, so what it works on. third-party: installed as a dependency. fixture: shipped by the repository\'s test setup, below a Tests/ directory, so it exists for a suite to load rather than for development.'],
                'deprecatedFiles' => Schema::listOf(Schema::deprecatedFile(), 'The files this extension ships that the core no longer reads, or stops to read, each with what the shipped file costs. Read for an extension of origin project alone and empty for the others, whose files are their own maintainer\'s. The check reads four predicates, each off the extension\'s own tree: ext_tables.php, ext_emconf.php, ext_icon.svg/.png/.gif and the two ext_typoscript_*.txt. An empty list says none of the four holds, not that the extension is ready for the next major. It reads nothing else it ships for a deprecation, and typo3_changelog_lookup answers that. typo3_extension_describe is the same verdict beside everything else that extension registers.'),
            ], ['key', 'path', 'origin', 'deprecatedFiles']), 'Extensions that are not TYPO3 system extensions. Read from Composer\'s installed metadata, so where installed is false this is empty because nothing sits installed rather than because the repository has none.'),
            ...
            'commands' => Schema::listOf(Schema::object([
                'command' => Schema::string('As this repository declares it, run from the project root. Where environment is not null, it runs inside that environment rather than in the caller\'s shell. An npm script declared below the root carries the --prefix that points npm at the manifest that declares it. So two manifests with a build script are two commands you can tell apart.'),
                'source' => Schema::string('The manifest that declares it, relative to the project root: composer.json, package.json, or Build/package.json where the repository keeps its frontend build one directory down.'),
                'invocation' => Schema::string('The same command as you run it from where you stand, which is what to paste. Where this repository configures a DDEV project and this server is not already inside it, it is the declared command with DDEV in front. That is "ddev composer <name>" for a composer script and "ddev exec <command>" for the rest. It is the declared command unchanged everywhere else, under TYPO3_DEV_COMPANION_CONSOLE too, which reaches this installation\'s console rather than an arbitrary script.'),
                'declares' => Schema::string('The body the manifest declares for it, lines joined with &&.'),
                'runs' => ['type' => 'string', 'enum' => ['check', 'change', 'unknown'], 'description' => 'What a run does to the sources, read off the body rather than measured by a run. check: it reports and hands the code back as it was, so a task told not to change files can run it. It may still write a cache of its own. change: it rewrites something. unknown: the body does not say, which is what a test suite is, because it runs the project\'s own code.'],
                'runThrough' => Schema::nullableString('The command this repository\'s own contract runs this one through instead, where it has one — the reason it is here rather than the declaration. A core checkout answers it on every npm script, because the core drives them through Build/Scripts/runTests.sh. That script supplies the PHP version and the database service a direct npm call does not, and the core\'s own AGENTS.md forbids the direct route. Null everywhere else, which means the declared command is the supported route.'),
            ...
            'uncheckedKinds' => Schema::listOf(Schema::string(), 'Kinds of file this project\'s own packages ship that no declared command names a checker for — "CSS", "PHP", "Sass", "TypeScript", "XLIFF". It says what is not covered and never what to add: which standards a repository holds itself to are its own. Read off the checkers named in the declared bodies. So a tool this server does not know contributes no coverage, and the list may name a kind that something unrecognised does check. JavaScript is never listed, because a .js a package ships is as often build output or a vendored library as source.'),
            ...
            'guides' => Schema::listOf(Schema::guideReference(), 'The whole procedures this server carries, named here because this is the call every task starts with. This server also serves them as typo3://guides resources, and a client that lists no resources renders none of them. Four sessions in one week finished without any knowledge that they exist. Each is one typo3_rule_lookup call by documentId, which needs no resource list. A search over sections answers a question and never hands one of these over whole.'),
