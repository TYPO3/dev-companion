...
final class TaskGuide extends ReadOnlyTool
{
    ...
    {
        return 'Answers what one change owes, which a repository\'s own conventions file cannot. That file states its rules once for every task. This narrows them to the kind of change, the paths and the TYPO3 majors in front of you. It goes down to whether this fix owes a changelog entry. Where such a rule stays general it names the call that settles it, the branches a Releases: trailer takes among them. The answer is a task checklist with the hints and core checks that match. Not only for work that ends in a patch. Whether an open bug report still holds is changeType "triage", and a review of a body of code is "audit". A boot of an installation is "operations", and the cause of a defect before anybody changes it is "diagnosis". All four get a brief of their own rather than the steps a patch owes. Built from bundled conventions only: it does not read your checkout, so it also names what you have to establish there yourself. It routes to the lookups that fit the task. It names the task skill that owns the work where a published one does. It names the guide that writes the work up where this server carries one. The hints for a set of paths without the checklist around them are typo3_hint_lookup, and a procedure read whole is typo3_rule_lookup. Work that reads as a project or third-party extension gets what transfers only. The answer leaves out the core checks, checklist items and steps that name something only the core repository has.';
    }
    ...
    public static function inputSchema(): array
    {
        return [
            ...
            'properties' => [
                ...
                'paths' => ['type' => 'array', 'items' => ['type' => 'string'], 'default' => [], 'description' => 'The files the task is about, as they are in the repository they belong to. Pass them where the work touches more than one place. The answer places each on its own, so a core path and an extension path in one call get two verdicts. An extension key counts as a path. A subsystem with no path to name belongs in task, because the answer reads every entry here as a file.'],
                'targetVersion' => ['type' => 'string', 'description' => 'The TYPO3 version this task is for, for example "13.4" or "14". The answer leaves out conventions that do not hold there, those the repository needs for another major it declares included. Defaults to every major this repository declares typo3/cms-core for, or to the installation this server started in where there is no declaration.'],
                'changeType' => ['type' => 'string', 'enum' => ['bugfix', 'feature', 'cleanup', 'test', 'documentation', 'deprecation', self::AUDIT, self::TRIAGE, self::OPERATIONS, self::DIAGNOSIS, 'unknown'], 'default' => 'unknown', 'description' => 'What kind of change the task is. Four of them write no file and get a brief of their own instead of the steps a patch owes. audit asks for what a review of a body of code needs. triage asks for what a decision on an open bug report needs. That is whether it still happens, what a previous attempt cost, and what a maintainer needs before it can move. operations asks for what a run of an installation needs. That is the boot of the environment a repository declares, the import of its data and the build of its assets. diagnosis asks for what the search for the cause of a reported defect needs, before anybody has agreed what to change. A review of a report against code, a review of a diff and the reason something is broken are three briefs and not one. A task that describes any of the four gets that shape without the type stated.'],
            ],
            'required' => ['task'],
        ];
    }
    ...
    public static function outputSchema(): array
    {
        return Schema::object([
            'task' => Schema::string(),
            'paths' => Schema::listOf(Schema::string(), 'The paths this brief is for. Empty where the call named none.'),
            'scopes' => Schema::scopes('Which kind of work each path is. Where every path is outside the core, the whole brief leaves out the core checks, the core-only checklist items and the submission route.'),
            'changeType' => Schema::string(),
            'targetVersion' => Schema::nullable(['type' => 'integer', 'description' => 'The TYPO3 major this repository runs — stated by the caller, or read from the installation. Null means no version filtered anything. Where the repository serves several majors, targetVersions is what the answer holds for.']),
            'targetVersions' => Schema::listOf(['type' => 'integer'], 'Every TYPO3 major the answer holds for. One entry is the ordinary case. Several mean this repository declares typo3/cms-core for more than one of them, so the answer keeps a statement that holds on any. Where two statements about the same subject differ, the difference is the constraint the code lives under rather than drift. Empty when no version filtered anything.'),
            ...
            'intents' => Schema::listOf(Schema::object([
                'id' => Schema::string(),
                'title' => Schema::string(),
                'confidence' => ['type' => 'string', 'enum' => ['strong', 'weak'], 'description' => 'weak: a word named the subject and not the work, or the intent is core-only and nothing in the task says this is core work. Either way it applies only under its condition.'],
            ...
            'staleSkills' => Schema::listOf(Schema::string(), 'Which of the skills above this project holds an older copy of, read from what stands published there against what this server writes now. Empty where every named copy is current, where this server never installed into this project, and where skills names none. So it is a subset of skills and never a statement that one is missing. What to do about it is one command, typo3-dev-companion update, and what it costs is a change to the caller\'s own checkout.'),
            'skills' => Schema::listOf(Schema::string(), 'The task skills that own the recognized work, named so that a caller who reached this server without one can load it. A skill is a file in your own project rather than something this server can see. So a name here is not a promise that the project holds it. A review, a triage, a boot and a diagnosis name only the workflows that change nothing either. intents still recognizes the kind of change under review, and the workflow that writes one is not the one you are in. Empty means no published skill owns what the brief recognized, which is not a statement that the work has no workflow.'),
            'guides' => Schema::listOf(Schema::guideReference(), 'The whole procedures that write this work up or that it owes. They are the corpus typo3_project_describe lists at orientation and this server serves as typo3://guides resources. Owes is the second half and the one a task text never asks for. A change that ends in a backend UI owes the browser check whether or not it says so. Named rather than carried: a brief is one call inside a procedure, and the page is one typo3_rule_lookup call by documentId. Empty means no page here writes up what the brief recognized. It does not say that none of them is worth a read; the whole list is in that orientation call.'),
            'hints' => Schema::listOf(Schema::hintRecord(), 'What typo3_hint_lookup answers for these paths, quoted whole and carried here — the strongest few per group of paths, not everything it holds on them. A hint that declares a different kind of repository from the paths given ranks below the ones that bind them. A rule taken from one of these belongs to that lookup. So a report that cites it names typo3_hint_lookup, and a caller who needs more of the subject calls it directly. omittedHints names what the brief left out.'),
            'omittedHints' => Schema::listOf(Schema::hintReference(), 'What typo3_hint_lookup also holds for these paths and this brief did not carry, named rather than counted. A hint declared for another kind of repository is here for that reason rather than for a weak match. Empty means what it carries is everything that matched. A subject listed here and not in hints is one the brief did not reach. It is the gap the pointer to that lookup stands for.'),
            ...
            'checks' => Schema::listOf(Schema::string(), 'Commands to run, ready to execute from the core root. They '
                ...
                . 'ones the recognized work names. This is the list to run. testSuites narrows the same corpus a '
                . 'second way, and a suite named there and not here is one to decide about.'),
            ...
            'testSuites' => Schema::listOf(Schema::testSuiteRecord(), sprintf(
                ...
                . 'whole list these come out of.',
                self::SUITES_PER_BRIEF,
            )),
            'beforeYouRun' => Schema::string('What a suite can take with it, and what to do before you start one. '
