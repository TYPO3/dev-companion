...
final class TerLookup extends ReadOnlyTool
{
    ...
    {
        return 'Read what the TYPO3 Extension Repository has published under an extension key, live from extensions.typo3.org. Pass extension with the key, the one extra.typo3/cms.extension-key declares and not the Composer package name. Every published version comes back, highest number first. That is the number, the state, the upload day, the TYPO3 majors it declares and the constraints.depends.typo3 of its release. This is the question a release audit cannot answer from the repository under audit. Tailor refuses to package unless ext_emconf.php names the version of the release. So that file still names it after the upload, and a published checkout reads exactly like one nobody has published. Pass extensionVersion as well to learn whether the registry already holds that number. It reports what the registry holds and judges no version free; the comparison against the working tree is yours. A key with nothing published under it gets that answer, which does not say that no such package exists. An extension that ships through Composer alone never registers here. What a release requires of the extension itself is typo3_hint_lookup with id="extension-ter-release". This tool reads only, with no credential. You register a key, upload a version and transfer an extension yourself, through Tailor and the token it carries.';
    }
        ...
        return [
            ...
            'properties' => [
                'extension' => [
                    'type' => 'string',
                    'minLength' => 1,
                    'description' => 'The extension key, for example "news" or "bootstrap_package". That is the key extra.typo3/cms.extension-key declares in the package\'s composer.json, which every TYPO3 extension package has to carry to install at all. It is not the Composer package name, which the registry does not take. "georgringer/news" and "bootstrap-package" are the two shapes that reach it as a name of the wrong kind. Lowercase letters, digits and underscores, three to thirty characters.',
                ],
                'extensionVersion' => [
                    'type' => 'string',
                    'minLength' => 1,
                    'description' => 'One version number to ask about, for example "14.0.1", which is usually the one ext_emconf.php names. The answer says whether the registry holds it and, where it does, what that release declared. The comparison reads it as the registry writes it, which is exactly three numbers. A suffix of any kind belongs to no published version, because the upload route accepts none.',
                ],
                'limit' => [
                    'type' => 'integer',
                    'minimum' => 1,
    ...
    public static function outputSchema(): array
    {
        return Schema::object([
            ...
            'url' => Schema::string('What the call read, so you can ask the same question again by hand. Empty where the key got its answer without a read.'),
            ...
            'versions' => Schema::listOf(Schema::object([
                ...
                'uploaded' => Schema::string('The upload day, as YYYY-MM-DD in UTC. The registry writes the same moment in its own timezone, so a release made late in the day carries the day before here.'),
                'majors' => Schema::listOf(Schema::integer(), 'The TYPO3 majors the release declares it runs on, lowest first.'),
            ...
            ], 'Why the registry answered nothing, where status says unavailable. Null otherwise, and null on a key with nothing published under it; that one is an answer.'),
