...
/**
 * Keeps one TYPO3 core checkout per covered version below .checkouts/. So a
 * check of what a statement holds for runs against this repository's own
 * sources. Not against whatever checkout happens to be on the machine.
 *
 ...
 *
 * The packages the core pins rather than contains stand here too. A statement
 * about one of them verifies against a tag of that package rather than against
 * a core branch, `D-KNW-106`. `Upkeep\PinnedPackage` says which line pairs with
 * which major.
 */
#[AsCommand(
...
final class CheckoutUpdate
{
     ...
     * line's newest tag.
     *
     * Two majors that pin the same line share one worktree, the core pins 9.x
     * on both 13.4 and 14.3. So what this creates follows the pins rather than
     * the version list.
     */
    ...
    /**
     * One treeless bare clone, created where it is absent and fetched to its
     * refs.
     */
    private static function mirror(OutputInterface $output, string $path, string $repository, bool $tags): bool
    {
        ...
        if ($tags) {
            // The release lines read at their tags, so a line that gained one
    ...
    private static function worktree(OutputInterface $output, string $mirror, string $path, string $ref): int
    {
        ...
        // Detached on the ref: these serve reads, never commits, so there is
